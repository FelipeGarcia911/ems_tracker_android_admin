...
public class HttpConnectionHelper {
    ...
    public void handleOnFailureRequest(byte[] responseBody) {
        if (responseBody != null) {
            try {
                JSONObject jsonObject = stringBuilderJSONObject(responseBody);
                String message = jsonObject.getString("message");
                String exception = jsonObject.getString("exception");
                String errorString = jsonObject.getString("error");
                Log.e("publish-onFailure", errorString + "--" + exception + "--" + message);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
